@props(['list', 'name', 'filter'])

<div x-data="{ open: false }" @click.away="open = false"
     class="flex flex-auto justify-center w-full md:flex-none md:w-auto"
>
        ...
        <div id="filter-dropdown" x-show="open" class="mx-auto z-10 mt-2 bg-white divide-y divide-gray-100 rounded-lg shadow w-full dark:bg-gray-700">
            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 overflow-y-auto max-h-56">
                @foreach($list as $item)
                    <li>
                        <a href="/?{{ $filter }}={{ $item->slug }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            {{ $item->name }}
                        </a>
                    </li>
                @endforeach
